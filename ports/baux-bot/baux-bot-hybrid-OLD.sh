...
MAX_CONTEXT_SIZE=8000  # Limit context to 8000 tokens to prevent timeouts
...
# Context size management function
track_context_size() {
    local current_context="$1"
    local current_size=${#current_context}
    if [ $current_size -gt $MAX_CONTEXT_SIZE ]; then
        echo "⚠️  Context size ($current_size) exceeds MAX_CONTEXT_SIZE ($MAX_CONTEXT_SIZE). Truncating..."
        echo "${current_context:0:$MAX_CONTEXT_SIZE}"
    else
        echo "$current_context"
    fi
}

# bauxd client functions for Phase 1 consolidation
bauxd_get() {
    local endpoint="$1"
    curl -s "http://localhost:9999$endpoint" -H "Content-Type: application/json" 2>/dev/null || echo '{"error": "bauxd connection failed"}'
}

bauxd_post() {
    local endpoint="$1"
    local data="$2"
    curl -s -X POST "http://localhost:9999$endpoint" \
         -H "Content-Type: application/json" \
         -d "$data" 2>/dev/null || echo '{"error": "bauxd connection failed"}'
}
...
register_with_bauxd() {
    ...
    local response=$(bauxd_post "/ai/services" "$service_data")
    echo "Registration response: $response"
}

discover_ai_services() {
    echo "🔍 Discovering AI services via bauxd..."
    local services=$(bauxd_get "/ai/services")
    echo "$services" | jq -r '.[] | select(.status == "active") | "\(.name) (\(.type)) - \(.node)"' 2>/dev/null || echo "jq not available, raw response: $services"
}
...
scan_directory_for_context() {
    ...
    for pattern in "${file_patterns[@]}"; do
        ...
        done < <(find "$scan_dir" -name "$pattern" -type f -printf '%T@ %p\n' 2>/dev/null | sort -rn | head -20 | cut -d' ' -f2- | tr '\n' '\0')
    done
}
...
# Function to apply improvements (basic implementation)
apply_improvement() {
    local improvement_text="$1"

    echo "🔧 Applying improvement..."

    # Extract code blocks from the improvement text
    local code_blocks
    code_blocks=$(echo "$improvement_text" | grep -A 100 '```bash' | grep -B 100 '```' | grep -v '```' || true)

    if [[ -n "$code_blocks" ]]; then
        echo "Found code blocks to apply..."
        # For now, just log what would be applied
        echo "Code to apply:"
        echo "$code_blocks"
        echo "⚠️  Manual application required - this is a placeholder"
    else
        echo "No code blocks found in improvement"
    fi

    echo "✅ Improvement application logged"
}
...
improve_self() {
    ...
    # Check if auto-apply is enabled for self-improvement loops
    if [[ "${AUTO_APPLY:-false}" == "true" ]]; then
        echo "🔄 AUTO_APPLY enabled - automatically applying improvements..."
        apply_improvement "$analysis"
        apply="y"
    else
...
main() {
    ...
    if [[ $# -gt 0 ]]; then
        echo "DEBUG: Command-line args: $# - '$1'"
        case "$1" in
            "register")
                register_with_bauxd
                exit 0
            ...
            "services")
                ...
                ;;
            "test")
                echo "BAUX-BOT bauxd integration test"
                echo "Functions available: bauxd_get, bauxd_post, register_with_bauxd, discover_ai_services"
                exit 0
                ;;
        esac
    fi
    ...
    echo "  'register' - Register with bauxd service"
    echo "  'services' - Discover AI services via bauxd"
